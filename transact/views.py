        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                ModificationRecords.objects.create(AchievementID=result,
                                                   StatusDescription=f"{user.name}提出交易申请，科研成果正在交易中")

                TransactionRecords.objects.create(AchievementID=result, InitiatorID=user, TransactionAmount=amount)
            ...
            form = TransactForm()
    return render(request, 'transact/submit_result.html', {'form': form, 'result': result, 'user': user})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if form.cleaned_data['agree_to_transact'] == 'True':
                ...
                # 更新科研成果所有权归属
                result.Ownership = transaction_record.InitiatorID
                # 更新科研成果定价
                ...
                buyer = transaction_record.InitiatorID
